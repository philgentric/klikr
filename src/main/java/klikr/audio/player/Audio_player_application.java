...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        Kontext context = new Kontext(stage_,aborter,Shared_services.logger());
        Integer reply_port = start_context.extract_reply_port();
        klikr_communicator = new Klikr_communicator("Audio_player_app",context);
        if (klikr_communicator.start_as_singleton())
        {
            ...
            if (selectedFile != null) {
                ...
                The_audio_player.play_song_in_folder(this,current, context);
            } else {
                current = null;
            }
            ...
            Consumer<String> on_appearance_changed = new Consumer<String>() {
                ...
                public void accept(String s) {
                    The_audio_player.play_song_in_folder(local_app,Path.of(s), context);
                    // path_list_provider;
                    //Window_builder.additional_no_past(this, Window_type.Song_playlist,path_list_provider, stage_, logger);
                    //Old_Audio_player_with_playlist.on_ui_changed(s,Shared_services.aborter(),context);
                }
            };
