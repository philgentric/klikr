...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        this.undo_core = new Undo_core("undos_for_music_playlist.properties", context);
    ...
    //**********************************************************
    {
        ...
        Runnable r = () ->
        {
            ...
            for (String path : the_list_of_new_songs)
            {
                ...
                if (f.isDirectory())
                {
                    load_folder(f, oks,to_be_renamed_first, context.aborter());
                }
                else
                {
                    sanitize(path,  oks,to_be_renamed_first);
                }
            }
            ...
            context.log(finaly.size()+ " files accepted as possible songs");
    ...
    //**********************************************************
    {
        ...
        {
            context.log("HAPPENS1 save_playlist");
        ...
        try
        {
            ...
            fw.close();
            context.log("Saved "+count+" songs in playlist file named:" + playlist_path.toAbsolutePath());
        }
        catch (IOException e)
        {
    ...
    //**********************************************************
    {
        ...
        Actor_engine.execute(r,"Update playlist length info",context.logger());
    }
    //**********************************************************
    private void update_playlist_size_info_in_a_thread()
    //**********************************************************
    {
        Aborter cleanup_aborter = new Aborter("update_playlist_size_info_in_a_thread", context.logger());
        Kontext local = new Kontext(context.owner(), cleanup_aborter,context.logger());
        double seconds[] = {0.0};
        for ( String path: the_playlist)
        {
            bitrate_cache.prefill_cache(Path.of(path),true,local);
            duration_cache.prefill_cache(Path.of(path),true,local);
        }

        for ( String path: the_playlist)
        {
            seconds[0] += duration_cache.get(Path.of(path),null,local);
        }
        ...
        /*
        final String text = My_I18n.get_I18n_string("Songs",context);
        Runnable r = () -> the_music_ui.set_total_duration(the_playlist.size() + " "+text+", " + get_nice_string_for_duration(seconds[0],context));
        Platform.runLater(r);
           */
    }

    //**********************************************************
    public String get_nice_string_for_duration(double seconds_in)
    //**********************************************************
    {
        int d = 0;
        int h = 0;
    ...
    //**********************************************************
    {
        ...
        for (File ff : files)
        {
            ...
            {
                sanitize(ff.getAbsolutePath(), oks, out);
            }
        }
    }

    //**********************************************************
    void sanitize(String song, List<String> oks, List<Old_and_new_Path> out)
    //**********************************************************
    {
    ...
    //**********************************************************
    {
        ...
        Look_and_feel_manager.set_button_look(b, true,context);
        ...
        b.setOnContextMenuRequested((ContextMenuEvent event) -> {
            //context.log("show context menu of button:"+ song.path);
    ...
    //**********************************************************
    {
        ...
        Menu_items.add_menu_item_for_context_menu(
                ...
                (ActionEvent e) ->
                        play_similar(Path.of(full_path), context),
                context_menu,
                context);
        ...
        Menu_items.add_menu_item_for_context_menu(
                ...
                (ActionEvent e) ->
                        Window_builder.additional_no_past(application, Window_type.File_system_2D, new Path_list_provider_for_file_system(Path.of(full_path).getParent(), context), context),
                context_menu,
                context);
...

        {
            ...
            Look_and_feel_manager.set_menu_item_look(the_menu_item,context);
            context_menu.getItems().add(the_menu_item);
            the_menu_item.setOnAction(e-> Audio_info_frame.show(Path.of(full_path),context));
        }

        Menu_items.add_menu_item_for_context_menu(
                "Edit_Song_Metadata",null,
                (ActionEvent e) -> Ffmpeg_metadata_editor.edit_metadata_of_a_file_in_a_thread(Path.of(full_path), context),
                context_menu, context);
...
/*
    //**********************************************************
    private void play_similar(Path path, Kontext context)
    //**********************************************************
    {
        Actor_engine.execute(()->
                play_similar_in_thread(path, context),"Find and play similar song",logger
        );
    }

    //**********************************************************
    private void play_similar_in_thread(Path path, Kontext context)
    //**********************************************************
    {
        long start = System.currentTimeMillis();
        ...
        if ( similars.size() == 0)
        {
            context.log("no similar song found");
            return;
        }
        Path similar = similars.get(0).path();
        context.log("similar song found : "+ similar);
...
/*
    ...
    //**********************************************************
    {
        if ( previous != null)
        {
            //context.log("change_song aborting previous");
            previous.abort("changing song");
        }
        if (new_song == null)
        {
            context.log(Logger.warning+"Warning new_song == null in change_song");
            return;
        }
    ...
    //**********************************************************
    {
        ...
        {
            context.log("change_song_in_a_thread aborting1");
            return;
        }
        ...
        if ( aborter.should_abort())
        {
            context.log("change_song_in_a_thread aborting2");
            return;
        }
        add_one_song_to_playlist_if_not_already_there(the_song_path);
        if ( aborter.should_abort())
        {
            context.log("change_song_in_a_thread aborting3");
            return;
        }
        the_music_ui.play_song_with_new_media_player(the_song_path, first_time);
        if ( aborter.should_abort())
        {
            context.log("change_song_in_a_thread aborting4");
            return;
        }
        set_selected(start);
    }
    ...
    //**********************************************************
    {
        ...
        if ( dbg) context.log(  (new File(new_song)).getName() + " (bitrate= " + bitrate + " kb/s)");
