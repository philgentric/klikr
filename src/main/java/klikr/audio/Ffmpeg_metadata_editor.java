...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        Platform.runLater(()->open_editor(path,context)),"Edit ffmpeg metadata",context.logger());
    }

    //**********************************************************
    private static String get_string_from_user(String key, String value,Kontext context)
    //**********************************************************
    {
    ...
    //**********************************************************
    private static void open_editor(Path path, Kontext context)
    //**********************************************************
    {
        ...
        String new_performer= get_string_from_user("performer (artist,group name etc)",performer,context);
        context.log ("new_performer="+new_performer);
        String new_title= get_string_from_user("title ('release', song name)",release,context);

        context.log ("new_title="+new_title);
        Runnable r = ()->
        {
            ...
            command_line_for_ffmpeg.add(External_application.Ffmpeg.get_command(context));
            ...
            context.log("command_line_for_ffmpeg:" + command_line_for_ffmpeg);

            StringBuilder sb = new StringBuilder();
            Execute_result res = Execute_command.execute_command_list(command_line_for_ffmpeg, path.getParent().toFile(), 2000, sb, context);
            if ( !res.status())
            {
            ...
            context.log(Logger.error+" " + sb);
            context.log("ffmpeg meta data edit:" + res.output());
        };
        Actor_engine.execute(r,"Ffmpeg meta data edit",context.logger());
    }
}
