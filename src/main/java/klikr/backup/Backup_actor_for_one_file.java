...
//**********************************************************
{
    ...
    //**********************************************************
    public Backup_actor_for_one_file(Backup_stats stats, Kontext context)
    //**********************************************************
    {
    ...
    //**********************************************************
    {
        ...
        if ( file_comparator==null) file_comparator = new File_comparator(context);
/*
 ...
 */       {
    ...
    //**********************************************************
    {
        if (verbose) context.log("           Doing the backup of file: "+fbjr.file_to_be_copied.getAbsolutePath());

        Dir_results local = duplicate_a_file(fbjr);
        if (!local.was_copied)
        {
            if ( fbjr.context.should_abort())
            {
                context.log("duplicate_a_file status is false, giving up");
    ...
    //**********************************************************
    {
        ...
        if (verbose ) context.log("Dealing with file ->" + file_backup_job_request.file_to_be_copied.getName());
        ...
        */
        ...
        {
            if (verbose == true)
                context.log("CREATE: no such file in archive yet, will create destination: " + destination_file.getAbsolutePath());
            if ( file_backup_job_request.mini_console != null)
            {
        ...
        if ( file_backup_job_request.check_for_same_file_different_name)
        {
            ...
            if (obsolete_name != null) {
                ...
                //try {
                    ...
                    context.log("renaming a destination file: " + obsolete_name.toFile().getName() + " to " + file_backup_job_request.file_to_be_copied.getName());
                    if ( !Moving_files.move_file(obsolete_name, source.resolveSibling(file_backup_job_request.file_to_be_copied.getName()),context))
                //} catch (IOException e)
                {
                    context.log("WARNING: attempt to rename failed ");
                    context.log("file_to_be_copied name =  " + file_backup_job_request.file_to_be_copied.getName());
                    context.log("destination_file name =  " + destination_file.getName());
                    returned.was_copied = true;
                    if ( file_backup_job_request.mini_console!=null) file_backup_job_request.mini_console.increment_file_count();
                    return returned;
                }
                context.log("updated the NAME of the file in the destination folder as it had the same content\n     old name :" +
        ...
        try
        {
            context.log("starting to COPY: " + file_backup_job_request.file_to_be_copied.getAbsolutePath() + " -> " + destination_file.getAbsolutePath());
            long b = File_copier.copy_file_NIO(destination_file, file_backup_job_request.file_to_be_copied);
            context.log("done COPY: " + file_backup_job_request.file_to_be_copied.getAbsolutePath() + " -> " + destination_file.getAbsolutePath());
            if ( file_backup_job_request.mini_console != null) file_backup_job_request.mini_console.add_to_copied_bytes(b);
        }
        catch (Exception eee) {
            context.log("EXCEPTION IN COPY: " + eee);
            if ( file_backup_job_request.mini_console != null)
            {
    ...
    //**********************************************************
    {
        File_with_a_few_bytes fr = new File_with_a_few_bytes(file_to_be_copied, context.logger());
        File[] files = destination_dir.listFiles();
        if ( files ==null) return null;
        for ( File f : files)
        {

            File_with_a_few_bytes ff = new File_with_a_few_bytes(f, context.logger());

            if ( File_with_a_few_bytes.files_have_same_content(fr,ff, context))
            {
                context.log("Found 2 files with same content:\n   "+file_to_be_copied.getAbsolutePath()+"\n   "+f.getAbsolutePath());
    ...
    //**********************************************************
    {
        switch (file_comparator.files_have_same_size(file_to_be_copied, destination_file))
        {
            default:
            case aborted:
                if (verbose == true) context.log("aborted: " + target_name);
                return Similarity_result.aborted;
            case same_size:
                if (verbose == true) context.log("SKIPPED: the SAME file is already there: " + target_name);
                if (deep)
                {
            ...
            case not_same:
                //if (verbose == true)
                context.log("OHOH ??: name exists, but files are DIFFERENT, checking again: " + target_name);
                ...
                {
                    context.log("fausse alerte: (deeper check)" + target_name);
                    return Similarity_result.same;
                }
                context.log("CONFIRMED: name exists, but files are DIFFERENT, destination will be renamed: " + target_name);
                // actual copy occurs below
                if ( !rename_destination_file(destination_file))
                {
                    context.log("WARNING: rename failed for:" + destination_file.getAbsolutePath());
                }
    ...
    //**********************************************************
    {
        ...
        {
            Path x = Moving_files.generate_new_candidate_name(destination_file.toPath(), "RENAMED_", ""+i, context.logger());
            if ( x.toFile().exists()) continue;
            //try {
                //FileUtils.moveFile(destination_file,x.toFile());
                if (!Moving_files.move_file(destination_file.toPath(),x,context))
        //    } catch (IOException e)
        {
                context.log_stack_trace("rename failed");
                return false;
            }
            return true;
        }
