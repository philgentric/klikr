...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        file_actor = new Backup_actor_for_one_file(stats, context);
    ...
    //**********************************************************
    {
        if ( dbg) context.log("Doing 1 folder:" + request.source_dir.getAbsolutePath());

        if (request.get_aborter().should_abort())
        {
            context.log("Aborting 1 folder:" + request.get_aborter().reason()+ " " + request.source_dir.getAbsolutePath()+ "=>"+ request.destination_dir.getAbsolutePath());
        ...
        {
            slow = true;
            mini_console = new Per_folder_mini_console(context);
            mini_console.create();
        ...
        if (!request.destination_dir.exists()) {
            if (request.destination_dir.mkdir()) {
                context.log("created folder: " + request.destination_dir);
        ...
        if (dbg) {
            context.log("******************************");
            context.log("will copy all content of dir: " + request.source_dir);
            context.log("                    into dir: " + request.destination_dir);
            context.log("                    should_abort ? " + request.context.should_abort());
            context.log("******************************");
        }
        ...
        for (File file_to_be_copied : all_source_files)
        {
            ...
            if ( Guess_file_type.should_ignore(file_to_be_copied.toPath(),context.logger())) continue;
            ...
            */
            file_actor.run(new File_backup_job_request(request.destination_dir, file_to_be_copied, mini_console, local_check_for_same_file_different_name, deep_byte_check,request.context));
            count++;
        }
        if ( dbg) context.log("Folder "+request.source_dir.getAbsolutePath()+" "+count+" files backups DONE");
        ...
        for (File sub_dir_to_be_copied : all_source_files)
        {
            ...
            {
                context.log("backup warning: symbolic link not followed "+sub_dir_to_be_copied);
                continue;
            }

            // create new job for this subfolder
            Directory_backup_job_request directory_backup_job_request = new Directory_backup_job_request(sub_dir_to_be_copied, new File(request.destination_dir, sub_dir_to_be_copied.getName()), context);
            ...
            if ( launch_in_thread)
            {
                actor_engine_based_on_workers.run(new Backup_actor_for_one_folder(stats, check_for_same_file_different_name,deep_byte_check,reports, actor_engine_based_on_workers,context), directory_backup_job_request, null, context.logger());
                threads_launched++;
            }
            else
            {
                if ( folder_actor == null) folder_actor = new Backup_actor_for_one_folder(stats, check_for_same_file_different_name,deep_byte_check,reports, actor_engine_based_on_workers,context);
                folder_actor.do_one_folder(directory_backup_job_request);
            }
            if (mini_console != null) mini_console.show_progress();

        }
        if ( dbg) context.log("Folder "+request.source_dir.getAbsolutePath()+" DONE");
