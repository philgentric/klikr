...
//**********************************************************
{
    ...
    //**********************************************************
    public static int get_random_active_port(ML_server_type server_type, Kontext context)
    //**********************************************************
    {
        if ( instance == null) instance = new Load_balancer();
        return instance.get_random_active_port_internal(server_type, context);
    }


    //**********************************************************
    private int get_random_active_port_internal(ML_server_type server_type, Kontext context)
    //**********************************************************
    {
        //boolean live_dbg = Feature_cache.get(Feature.Enable_ML_server_debug);
        ML_servers_status status = find_active_servers(server_type, context);
    ...
    //**********************************************************
    private ML_servers_status find_active_servers(ML_server_type server_type, Kontext context)
    //**********************************************************
    {
        List<ML_server> servers_of_type = ML_registry.get_servers_of_type(server_type, context);
        ML_server_launch_status status = ML_server_launch_status.RUNNING;
        if (servers_of_type.size() <server_type.quota(context.owner()))
        {
            if ( dbg) context.log("\n\nfind_active_servers_internal server type under quota "+server_type.name());
            if (!server_pump_started.get())
            {
                if ( dbg) context.log("find_active_servers_internal starting server creation pump");
                start_server_creation_job(context);
            }
            // send a request for more servers
            status = ML_server_launch_status.STARTING;
            if ( request_queue.offer(server_type))
            {
                if ( dbg) context.log("find_active_servers_internal request sent");
            }
            else
            {
                if ( dbg) context.log("find_active_servers_internal request denied (queue is full)");
            }
        ...
        if ( dbg) context.log("find_active_servers_internal "+active_servers_ports.size()+" active servers detected");
        return new ML_servers_status(active_servers_ports, status);
    }

    //**********************************************************
    private void start_server_creation_job(Kontext context)
    //**********************************************************
    {
        boolean live_dbg = Feature_cache.get(Feature.Enable_ML_server_debug);
        pump_aborter = new Aborter("server_creation_job_aborter", context.logger());
        ...
        Runnable r = () -> {
            for(;;) {
                try {
                    ...
                    if (servers_of_type.size() >= server_type.quota(context.owner()))
                    {
                        if ( live_dbg) context.log("\n\n"+ Logger.ok+" No need to start servers of type: "+server_type.name()+ " running: "+servers_of_type.size()+ " quota: "+server_type.quota(context.owner()));
                        continue;
                    }
                    ...
                    int full_quota = server_type.quota(context.owner());
                    ...
                    if ( more > 0)
                    {
                        ...
                        ML_servers_util.start_N_ML_servers(more, server_type, context);
                    }
                    else
                    {
                    ...
                    context.log("Interrupted while waiting for servers to start");
                }
            }
        };
        Actor_engine.execute(r,"ML_server_launch", context.logger());
    }
