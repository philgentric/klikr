...
//**********************************************************
{
	...
	//**********************************************************
	{
		...
		deduplication_by_similarity_engine = deduplication_by_similarity_engine_;
		Path_list_provider path_list_provider = new Path_list_provider_for_file_system(deduplication_by_similarity_engine.target_dir.toPath(),context);
	...
	//**********************************************************
	{
		...
		if ( dbg) context.log("Runnable_for_finding_duplicate_file_pair_similarity RUN starts");
		int ignored = 0;
		...
		if ( dbg) context.log("image_properties_cache = "+image_properties_cache);
		for (File_with_a_few_bytes f : all_files)
		{
			...
			if (dbg) context.log("similars fond:\n     " + f.file.getAbsolutePath() + "\n    " + most_similar.path());
