...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        {
            context.log("path manager aborting2");
            return;
        }
        ...
        if (Guess_file_type.is_this_extension_a_video(extension))
        {
            if (show_video_as_gif)
            {
                if ( extension.equalsIgnoreCase("MKV"))
                {
                    ...
                    if ( Guess_file_type.does_this_file_contain_a_video_track(path,context))
                    {
        ...
        if ( context.should_abort())
        {
            context.log("path manager aborting3");
            return;
        }
        ...
        if ( context.should_abort())
        {
            context.log("path manager aborting4");
            return;
        }

        if (Guess_file_type.is_this_extension_an_image(extension))
        {
            if ( Sort_files_by.need_image_properties(path_list_provider.get_key(),context))
            {
                if (dbg) context.log("calling image properties cache prefill from path holder");
                // calling this will pre-populate the property cache
                // if needed
                image_properties_cache.prefill_cache(path, true,context);
            }
            iconized_paths.add(path);
            return;
        }
    ...
    //**********************************************************
    {
        ...
        for (Path p : folders)
        {
            Static_files_and_paths_utilities.remove_empty_folders(p, recursively, context);
        }
    }
