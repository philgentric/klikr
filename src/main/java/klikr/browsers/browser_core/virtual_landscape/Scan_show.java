...
//**********************************************************
{
    ...
    //**********************************************************
    Scan_show(Scan_show_slave scan_show_slave_, Vertical_slider slider, Kontext context)
    //**********************************************************
    {
        ...
        if ( dbg) context.log("slider_unit_per_row="+slider_unit_per_row);
        MIN_SPEED = slider_unit_per_row/300.0;
        if ( dbg) context.log("MIN_SPEED="+MIN_SPEED);
        MAX_SPEED = slider_unit_per_row/3.0;
        if ( dbg) context.log("MAX_SPEED="+MAX_SPEED);
    ...
    //**********************************************************
    {
        ...
        private_aborter = new Aborter("browser scan show aborter", context.logger());
        Runnable r = () -> {
            if ( dbg) context.log("scan show start runnable");
            for(;;)
            {
                ...
                {
                    if ( dbg) context.log("scan show too SLOW, skipping sleep AND delta");

                    last_start_sleep_ms = now;
                    continue;
                }

                if ( dbg) context.log("scan show sleep for "+inter_frame_ms+" ms");
                try {
                ...
                Runnable r1 = () -> {
                    ...
                    {
                        if ( dbg) context.log("scan scroll done = "+dy);
                    }
                    else
                    {
                        if ( dbg) context.log("inverting scan direction !");
        ...
        Actor_engine.execute(r,"scan show",context.logger());
         ...
         */
        if ( dbg) context.log("scan show start " + inter_frame_ms);
    ...
    //**********************************************************
    {
        ...
        if ( private_aborter != null)
        {
            ...
            private_aborter = null;
            if ( dbg) context.log("scan show stop");
        }
    }
    ...
    //**********************************************************
    {
        ...
        if ( dbg) context.log("new scan show speed:"+dy);
        start_the_show();
