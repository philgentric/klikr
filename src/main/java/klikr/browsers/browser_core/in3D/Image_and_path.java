...
//*******************************************************
{
    ...
    //*******************************************************
    public Image_and_path( Path path, int small_image_size, int large_image_size, Kontext context)
    //*******************************************************
    {
        this.path = path;
...
//        return large_image;
    ...
    //*******************************************************
    {
        ...
        {
            context.log(path+ " PANIC loaded image is broken");
            return null;
        }
    ...
    //*******************************************************
    {
        if (Files.isDirectory(path))
        {
            //context.log(path.toAbsolutePath() +" is folder");
            String relative_path = "icons/wood/folder.png";
            Image folder_icon = Jar_utils.load_jfx_image_from_jar(relative_path, icon_size, context.logger());

            //Image folder_icon = Look_and_feel_manager.get_folder_icon(icon_size,context);
        ...
        {
            context.log("WARNING, displaying non-image files not implemented in 3D "+path.toAbsolutePath());
            return Jar_utils.get_broken_icon(icon_size,context.logger());
        }
        //context.log(path.toAbsolutePath() +" is image");

        try (InputStream is = new FileInputStream(path.toFile())) {
            return new Image(is, icon_size, icon_size, true, true);
        } catch (Exception e) {
            context.log(Logger.error+" FATAL " + e);
            return null;
        }
    }
    ...
    //*******************************************************
    {
        ...
        if (split)
        {
            ...
            for (;;) {
                ...
                //context.log("wrote ->" + piece + " at: " + tx + " , " + ty);
                ty += txtHeight;
            }
        }
        else
        {
            ...
            gc.drawImage(txtImg, tx, ty);
            //context.log("wrote ->" + path_string + " at: " + tx + " , " + ty);
        }
