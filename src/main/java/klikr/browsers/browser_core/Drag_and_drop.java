...
import klikr.browsers.browser_core.items.Item;
...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        if (source == null)
        {
            context.log((Logger.warning+" WARNING: accept_drag_dropped_as_a_move_in, cannot check for stupid move because the event's source is null: " + drag_event.getSource()));
        }
        else
        {
            if (source == excluded) {
                context.log(Logger.warning+" source is excluded: cannot drop onto itself");
                drag_event.consume();
                return 0;
            }
            //context.log(Stack_trace_getter.get_stack_trace("source class is:" + source.getClass().getName()));
            //context.log("excluded class is:" + excluded.getClass().getName());
            if (source instanceof Item item)
            {
                Node node_of_source = item.get_Node();
                context.log("excluded:" + excluded);
                // data is dragged over the target
                // accept it only if it is not dragged from the same node
                if (node_of_source == excluded) {
                    context.log("drag reception for stage: same scene, giving up<<");
                    drag_event.consume();
                    return 0;
                }
            }
        ...
        if (s == null) {
            context.log( "dragboard.getString()== null");
        }
        else
        {
            if ( drag_and_drop_dbg) context.log(origin + " drag ACCEPTED for STRING:->" + s+ "<-");
            for (String ss : s.split("\\r?\\n"))
            {
                if (ss.isBlank()) continue;
                if ( drag_and_drop_dbg) context.log(origin + " drag ACCEPTED for additional file: " + ss);
                the_list.add(new File(ss));
            }
            if (the_list.isEmpty())
            {
                context.log(origin + " drag list is empty ? " + s);
            }
        }
        {
            ...
            for (File fff : l)
            {
                if ( drag_and_drop_dbg) context.log(origin + "... drag ACCEPTED for file= " + fff.getAbsolutePath());
        ...
        for ( File f : the_list)
        {
            ...
            if (drag_and_drop_dbg) context.log("going to drag/move:"+f.getAbsolutePath());
        }
        if ( dir_count >= 2)
        {
            ...
            context.log("popup alert to warn user");
    ...
    //**********************************************************
    {
        node.setOnDragEntered(drag_event -> {
            if (Drag_and_drop.drag_and_drop_dbg) context.log("OnDragEntered RECEIVER SIDE" );
            Look_and_feel_manager.set_background_for_setOnDragEntered(node,context.logger());
            drag_event.consume();
        });
        node.setOnDragExited(drag_event -> {
            if (Drag_and_drop.drag_and_drop_dbg) context.log("OnDragExited RECEIVER SIDE");
            Look_and_feel_manager.set_background_for_setOnDragExited(node,context.logger());
            drag_event.consume();
        });
        node.setOnDragOver(drag_event -> {
            if (Drag_and_drop.drag_and_drop_dbg) context.log("OnDragOver RECEIVER SIDE");

            drag_event.acceptTransferModes(TransferMode.MOVE);
            //GLUON drag_event.acceptTransferModes(TransferMode.COPY);
            Look_and_feel_manager.set_background_for_setOnDragOver(node,context.logger());
            drag_event.consume();
        });
        node.setOnDragDropped(drag_event -> {
            if (Drag_and_drop.drag_and_drop_dbg) context.log("OnDragDropped RECEIVER SIDE");
    ...
    //**********************************************************
    {
        node.setOnDragDetected(drag_event -> {
            if (drag_and_drop_dbg) context.log("Item.init_drag_and_drop() drag detected SENDER SIDE");
            ...
            {
                context.log("Item.init_drag_and_drop_SENDER_SIDE: selection_handler.isPresent()");
                ll.addAll(selection_handler.get_selected_files());
            }
            context.log("Item.init_drag_and_drop_SENDER_SIDE: " + ll.size() + " files selected");
            // if we are here it is because the user is dragging an item
            if (!ll.contains(path))
            {
                ll.add(path);
                context.log("Item.init_drag_and_drop_SENDER_SIDE: added the ONE target file: " + path.toAbsolutePath() );
            }
            ...
            if ( Drag_and_drop.drag_and_drop_dbg) context.log(" selected files: " + sb);
        ...
        node.setOnDragDone(drag_event -> {
            ...
            {
                if (drag_and_drop_dbg) context.log("Item.init_drag_and_drop() SENDER SIDE: setOnDragDone for " + path.toAbsolutePath());
                /*
