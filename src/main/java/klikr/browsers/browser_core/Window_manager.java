...
//**********************************************************
{
    ...
    //**********************************************************
    public static void unregister(int id, Kontext context)
    //**********************************************************
    {
        alive.remove(id);
        context.log("closing a window, remaining ="+alive.size());
        if (alive.isEmpty())
        {
            //if ( dbg)
            context.log("last window dies");
            ...
            //if (dbg)
            context.log("last window closing GOING TO CALL Platform.exit()");
            Platform.exit();
            //if (dbg)
            context.log("last window closing GOING TO CALL System.exit()");
            System.exit(0);
        }
        else
        {
            if ( dbg) context.log("number_of_windows > 0");
        }
    }
