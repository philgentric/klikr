package klikr.browsers.browser_core.diskview;
...
//*******************************************************
{
    ...
    //*******************************************************
    public Draw_command(File_node node, Rectangle2D bounds, int color_family, int depth, long root_total_size, Kontext context)
    //*******************************************************
    {
        this(node, bounds, color_family, depth, root_total_size, false, context);
    }

    //*******************************************************
    public Draw_command(File_node node, Rectangle2D bounds, int color_family, int depth, long root_total_size, boolean placeholder, Kontext context)
    //*******************************************************
    {
    ...
    //*******************************************************
    public void execute(Pane pane, Application application)
    //*******************************************************
    {
        double bw = bounds.getWidth();
        double bh = bounds.getHeight();
        ...
        rect.setOnMousePressed(ev -> {
            if (ev.isSecondaryButtonDown()) {
                ContextMenu contextMenu = create_ContextMenu(node, rescan_callback,application, context);
    ...
    //*******************************************************
    private static ContextMenu create_ContextMenu(File_node node, Runnable rescan_callback, Application application, Kontext context)
    //*******************************************************
    {
        ContextMenu context_menu = new ContextMenu();
        Virtual_landscape.attach_contextmenu_to_browser(context.owner(), context_menu);

        {
            if ( !node.is_this_a_directory())
            {
                Path p = node.get_file().toPath();
                if (Guess_file_type.is_this_path_extension_an_image(p,context))
                {
                    Menu_items.add_menu_item_for_context_menu(
                        ...
                        event -> {
                            Image_window.get_Image_window(p, new Path_list_provider_for_file_system(p.getParent(),context),null, context);
        ...
        }
        {
            Menu_items.add_menu_item_for_context_menu(
                    ...
                    event -> {
                        ...
                        Window_builder.additional_no_past(application, Window_type.File_system_2D,new Path_list_provider_for_file_system(folder_path,context), context);
                    }, context_menu, context);
