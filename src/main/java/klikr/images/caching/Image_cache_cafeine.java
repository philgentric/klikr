...
//**********************************************************
{
    ...
    //**********************************************************
    public Image_cache_cafeine(int forward_size_, Kontext context)
    //**********************************************************
    {
        ...
        cache = Caffeine.newBuilder()
                ...
                /*.removalListener((key,value,cause)->{
                    if ( cause.wasEvicted())
                    {
                        context.log("Cafeine evisted: "+key+" cause:"+ cause);
                    }
                })*/
                .build();
    ...
    //**********************************************************
    {
        if (dbg) context.log("writing in Caffeine:" + value.path.getFileName());
        cache.put(key, value);
    }
    ...
    //**********************************************************
    {
        ...
        if (Check_remaining_RAM.RAM_running_low("full-length image cache preload", context))
        {
            //if (ultra_dbg)
        ...
        //if (ultra_dbg) context.log("preloading target: " + how_many_preload_to_request);
    ...
    //**********************************************************
    {
        ...
        for (Map.Entry e : cache.asMap().entrySet())
        {
            ...
            if ( image_context_owner.image_indexer.distance_larger_than(forward_size,image_context_owner.get_image_context().get().path,local_image_context.path, context.aborter()))
            {
                cache.invalidate(key);
                if (ultra_dbg) context.log("       Evicted:" + key + ", distance too large from:" + image_context_owner.get_image_context().get().path + " to " + local_image_context.path.toAbsolutePath());
            }
            else
            {
                if (ultra_dbg)
                    context.log("       NOT evicted:" + key );
            }
        }
        if (ultra_dbg) context.log("---------- end cache content: -------------");
    ...
    //**********************************************************
    {
        ...
        cache.invalidate(key);
        if (ultra_dbg) context.log("       Evicted:" + key );
    }
    ...
    //**********************************************************
    {
        ...
        for ( Map.Entry<String ,Image_context> e : m.entrySet())
        {
            ...
            context.log("   cache entry: "+ic.path);
            total_pixel += ic.image.getHeight()*ic.image.getWidth();
            count++;

        }
        context.log(count+ " images in cache");
        context.log("Total cache length: "+total_pixel/1_000_000+" Mpixels");
        context.log("cache hitRate: "+s.hitRate());
        context.log("cache loadCount: "+s.loadCount());
        context.log("cache totalLoadTime: "+s.totalLoadTime());
